...
object PoTokenProviderImpl : PoTokenProvider {
    ...
    override fun getWebClientPoToken(videoId: String): PoTokenResult? {
        Logger.printInfo { "PoTokenProviderImpl: getWebClientPoToken called for videoId=$videoId" }
        if (webViewBadImpl) {
            Logger.printInfo { "PoTokenProviderImpl: Returning null - webViewBadImpl is true" }
            return null
        }

        try {
            val result = getWebClientPoToken(videoId = videoId, forceRecreate = false)
            Logger.printInfo { "PoTokenProviderImpl: Got PoToken result for videoId=$videoId" }
            return result
        } catch (e: RuntimeException) {
            Logger.printException({ "PoTokenProviderImpl: RuntimeException for videoId=$videoId" }, e)
            // RxJava's Single wraps exceptions into RuntimeErrors, so we need to unwrap them here
            when (val cause = e.cause) {
                is BadWebViewException -> {
                    Log.e(TAG, "Could not obtain poToken because WebView is broken", e)
                    Logger.printInfo { "PoTokenProviderImpl: WebView is broken, setting webViewBadImpl=true" }
                    webViewBadImpl = true
                    return null
                }
                null -> throw e
            ...
            Logger.printException({ "PoTokenProviderImpl: Non-runtime exception for videoId=$videoId" }, e)
            throw e
        }
    }
    ...
    private fun getWebClientPoToken(videoId: String, forceRecreate: Boolean): PoTokenResult {
        Logger.printInfo { "PoTokenProviderImpl: getWebClientPoToken(private) called, forceRecreate=$forceRecreate" }
        ...
        val (poTokenGenerator, visitorData, streamingPot, hasBeenRecreated) =
            synchronized(WebPoTokenGenLock) {
                ...
                Logger.printInfo { "PoTokenProviderImpl: shouldRecreate=$shouldRecreate (generator=${webPoTokenGenerator != null})" }

                if (shouldRecreate) {
                    try {
                        Logger.printInfo { "PoTokenProviderImpl: Getting client version..." }
                        val innertubeClientRequestInfo = InnertubeClientRequestInfo.ofWebClient()
                        innertubeClientRequestInfo.clientInfo.clientVersion =
                            YoutubeParsingHelper.getClientVersion()
                        Logger.printInfo { "PoTokenProviderImpl: Client version: ${innertubeClientRequestInfo.clientInfo.clientVersion}" }

                        Logger.printInfo { "PoTokenProviderImpl: Getting visitor data from InnerTube..." }
                        ...
                        Logger.printInfo { "PoTokenProviderImpl: Got visitor data: ${webPoTokenVisitorData?.take(20)}..." }

                        // close the current webPoTokenGenerator on the main thread
                        webPoTokenGenerator?.let { Handler(Looper.getMainLooper()).post { it.close() } }

                        Logger.printInfo { "PoTokenProviderImpl: Creating PoTokenWebView generator..." }
                        // create a new webPoTokenGenerator
                        webPoTokenGenerator = PoTokenWebView
                            .newPoTokenGenerator(Utils.getContext()).blockingGet()
                        Logger.printInfo { "PoTokenProviderImpl: PoTokenWebView generator created" }

                        // The streaming poToken needs to be generated exactly once before generating
                        // any other (player) tokens.
                        Logger.printInfo { "PoTokenProviderImpl: Generating streaming PoToken..." }
                        webPoTokenStreamingPot = webPoTokenGenerator!!
                            .generatePoToken(webPoTokenVisitorData!!).blockingGet()
                        Logger.printInfo { "PoTokenProviderImpl: Streaming PoToken generated" }
                    } catch (e: Exception) {
                        Logger.printException({ "PoTokenProviderImpl: Failed during initialization" }, e)
                        throw e
                    }
                }
